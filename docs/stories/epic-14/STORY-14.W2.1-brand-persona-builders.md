...
- [ ] `brand create` delega ao `build_brand_pack.py` existente, sem implementar
  um segundo contrato ou builder paralelo.
- [ ] Existe `build_persona_pack.py` e o subcomando `persona create` usa o mesmo
  loader do runtime para validar o resultado.
- [ ] Persona exige nome, papel, descrição, voz, tópicos, guardas, ao menos uma
  foto, rights notice e consent reference.
- [ ] Cada foto recebe SHA-256 e permanece associada ao consentimento informado.
- [ ] Assets são copiados para diretório temporário e o pack só substitui o
  destino após validação completa.
- [ ] O destino padrão fica sob
  `projetos/{slug}/creative-factory/packs/{pack-id}/`.
- [ ] Path fora da raiz do projeto, destino não vazio e symlink de escape falham
  fechado.
- [ ] `--dry-run` gera somente plano/relatório e não cria o pack.
- [ ] `--json` emite resumo sanitizado; `--force` é explícito e auditável.
- [ ] Brand e persona geram `pack.json`, `preview.html` e
  `build-report.json` consistentes.
- [ ] O builder não presume licença pública nem redistribuição de assets.
- [ ] Testes cobrem idempotência, rollback atômico, foto adulterada, ausência de
