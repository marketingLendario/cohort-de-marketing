...
Na ponte da Aula 2 para a Aula 3, o aluno usa somente **`/iniciar-trafego`**. A skill descobre ou seleciona o projeto por identidade, chama internamente o worker `/iniciar-studio`, sincroniza antes de qualquer decisão de interface e faz uma única pergunta: **“Quer abrir?”**. Nunca pergunte se quer sincronizar nem instrua um segundo comando para iniciar o Studio.

## Skills disponíveis

`comecar` · `avatar-funil` · `espiao-do-concorrente` · `trend-hunting` · `swipe-file` · `offerbook` · `design-md` · `metodo-funil` · `copy-funil` · `quiz-funil` · `webinario-funil` · `vsl-funil` · `advertorial-funil` · `lancamento-funil` · `pagina-vendas-funil` · `email-funil` · `whatsapp-funil` · `conteudo-funil` · `criativos-funil` · `mockup-produto-funil` · `bonus-funil` · `backend-funil` · `recuperacao-funil` · `cro-funil` · `status-funil` · `iniciar-trafego` · `iniciar-studio` · `zelador` · `briefista` · `estruturador` · `ads-creative-factory` · `leitor-de-metricas` · `diagnosticador` · `analista-de-dados` · `coletor-de-dados` · `board-de-especialistas` · `painel-de-dados` · `retroalimentacao` · `gestor-de-campanhas`
