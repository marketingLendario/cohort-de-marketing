...
import { assertDocumentPackProposal, documentPackPrompt, loadDocumentPackContract } from './document-pack/contracts.js';
  ...
  async run(
    ...
    const retryCorrection = options.previousFailureReason
      ? [
          ...
          `Diagnóstico da tentativa anterior: ${[...options.previousFailureReason]
            .map((character) => {
    ...
    try {
      ...
      const elicitationQuestions = proposal.questions.map((question) => question.trim()).filter(Boolean);
      if (proposal.questions.length !== elicitationQuestions.length || elicitationQuestions.length > 10) {
        throw new Error(`${skill.title} retornou perguntas de elicitação inválidas.`);
      }
