...
const CONTRACTS = join(ROOT, 'data/contracts/iniciar-studio');
const FIXTURES = join(CONTRACTS, 'fixtures');

function fail(msg) {
  console.error(`validate-iniciar-studio-schemas: FAIL — ${msg}`);
  process.exit(1);
}
...
console.log(`validate-iniciar-studio-schemas: PASS (${index.contracts.length} schemas, ${index.contracts.length * 2} fixtures)`);
process.exit(0);
