...
describe('LocalFirstRun', () => {
  ...
  it('creates the access and signs in through Supabase', async () => {
    ...
    render(<LocalFirstRun />)
    await screen.findByRole('heading', { name: 'Primeiro acesso local' })
    ...
    await user.type(screen.getByLabelText('Nome do workspace'), 'Meu workspace')
    await user.click(screen.getByRole('button', { name: 'Criar acesso local' }))
